document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (inputs.apellido.value.trim().length < 2) {
            showError(inputs.apellido, 'El apellido debe tener al menos 2 caracteres.');
            isValid = false;
        } else if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(inputs.apellido.value.trim())) {
        ...
        if (telValue !== '' && !/^[6789]\d{8}$/.test(telValue)) {
            showError(inputs.telefono, 'El teléfono debe ser un número válido de 9 dígitos.');
            isValid = false;
        }

        if (inputs.mensaje.value.trim().length < 10) {
            showError(inputs.mensaje, 'El mensaje debe tener al menos 10 caracteres.');
            isValid = false;
        }
